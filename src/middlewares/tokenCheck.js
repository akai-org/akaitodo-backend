...
const tokenCheck = (req, res, next) => { // Guard
    ...
    try{
        ...
        if(err && err.name=="JsonWebTokenError" && secret.getSecretKeyPrevious()){
            jwt.verify(token, secret.getSecretKeyPrevious(), function(err2, decoded2) {
                if(!err2){
        ...
        if(decoded.iat+(process.env.TOKEN_REFERESH_TIME|0) < (new Date().getTime()/1000)){
...
function getToken(tid){
    return jwt.sign({
        id: tid,
      }, secret.getSecretKeyNow(),{ expiresIn: process.env.TOKEN_EXPIRATION_TIME|0 });
}
